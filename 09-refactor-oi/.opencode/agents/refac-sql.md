...
| 첫 인자 | 본문은 어디에 | 당신이 할 일 |
|---|---|---|
| `DPICALL` | `<작업폴더>/src-sql/` 의 mapper XML | 읽고 판정합니다 |
| `SQLEXEC` | `<작업폴더>/src/` — 화면이 `_sql` 에 조립해 둡니다 | 읽고 판정합니다. **ID 가 없는 게 정상**입니다 |
| **그 밖의 전부** | **어디에도 없습니다** (Rule 시스템) | **그냥 지나갑니다** |

`SET_SIMAXDATA` · `LOTCOMMENT` · `TKIN` · `TKOUT` · `ISSUE` … 는 백엔드 API 호출이고
로직이 Rule 시스템에 있습니다. **저장소에 안 들어옵니다.**

**`SET_SIMAXDATA` 를 조심하세요** — 뒤에 붙는 `"legacy_semis.updateSemisDelivery"` 가
SQL ID 와 똑같이 생겼지만 mapper 에 없습니다. mapper 에서 찾지 말고, 검색기로
밖을 뒤지지도 말고, 「정의 없음」으로도 「확인 못 한 것」으로도 올리지 마세요 —
확인 대상이 아닙니다. 인덱서가 `sql.ruleMessages` 로 이미 갈라 두었습니다.
