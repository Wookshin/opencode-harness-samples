...
`1-index.json` 의 확신(`높음`·`중간`·`낮음`)을 `영향` 칸에 **그대로 옮겨 적습니다.**
읽는 사람이 그것으로 지울지 말지를 정합니다.

| 확신 | 어떻게 씁니다 |
|---|---|
| **높음** | "지워도 컴파일과 동작이 그대로입니다" |
| **중간** | **"저장소 전체에서 찾아본 뒤 지우세요"** — `public` 이라 경로 밖에서 쓸 수 있습니다 |
| **낮음** | **"사람이 확인해야 합니다"** — 지우라고 단정하지 마세요 |

`중간`·`낮음` 은 그 한계를 `확인 못 한 것` 에도 남기세요.

## 산출물

`<작업폴더>/2-suggest-hygiene.md` 에 `suggest-format.md` 형식 그대로 씁니다.
제안 ID 는 **`H001`** 부터. 근거는 **`K-1`** · **`P-1`** 같은 체크리스트 번호를 답니다.
개선 유형은 `K-*` 면 **삭제**, `P-1`·`P-2` 면 **중복 통합**, `P-3` 이면 **상수화** 입니다.
...
인덱스의 중복 후보에 **`rvWrapper: true`** 가 붙은 쌍이 있습니다.
`_param.Clear()` → `_param.Add(…)` → `SendMessageWithJSON(…)` 형태입니다.

**그 쌍은 제안으로 올리지 마세요.** 호출 하나가 기능 하나라 같은 모양일
수밖에 없고, `ExecuteLotQuery(sqlId, lotId)` 같은 범용 헬퍼로 합치면
`GetCurLineId()` 라는 이름이 말해 주던 기능이 SQL ID 문자열 뒤로 숨습니다.
**글자는 줄지만 읽기는 나빠집니다.**

이 코드베이스는 반대 방향을 씁니다 — 기능마다 함수를 만들고 **이름에 그 기능을
담습니다.** 자세한 것은 `hygiene-rules.md` 의 `P-1` 입니다.

## 금지

- 코드를 고치지 마세요. 권한으로도 막혀 있습니다.
- **인덱스에 없는 미참조·중복을 만들어 내지 마세요.** 가장 흔한 반려 사유입니다.
- **`rvWrapper: true` 인 쌍을 「합치세요」로 올리지 마세요.**
- `1-units.md` 표에 없는 심볼을 제안하지 마세요.
- 확신 `중간`·`낮음` 을 **「지우세요」로 단정하지 마세요.** 검증에서 `NEEDS-INFO` 가 됩니다.
