...
직접 잘라 씁니다. 당신은 좌표(`unitId` · `line` · `currentLines`)만 넘깁니다.

제안자가 `**지금 코드**` 절 머리에 적은 줄 범위(`1233–1240` · `1250–1256`)를 그대로
`"currentLines": [[1233, 1240], [1250, 1256]]` 로 옮기세요. 붙어 있던 코드는
**버립니다** — 스크립트가 그 좌표로 원문에서 다시 잘라 오고, 같은 좌표가
「줄 보기」의 강조 범위도 됩니다.

`**제안 코드**` 의 ``` 펜스는 **손대지 말고 `suggestion` 에 그대로** 넣으세요.
리포트가 펜스를 갈라 접이식 코드 블록 + 복사 버튼으로 그립니다. 펜스를 풀면
그 기능이 사라집니다.
...
- **코드 원문을 JSON 에 옮겨 적지 마세요.** `currentLines` 는 **좌표**입니다.
  `current` 문자열은 옛 JSON 을 위해 남아 있을 뿐이니 새로 쓰지 마세요.
- **`suggestion` 의 ``` 펜스를 풀지 마세요.** 접이식과 복사 버튼이 사라집니다.
