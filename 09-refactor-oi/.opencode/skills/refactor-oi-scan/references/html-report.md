...
「지금」도 같습니다. `currentLines` 에 **좌표만** 적으면 스크립트가 원문에서 잘라
줄번호와 함께 그립니다. 그리고 **같은 좌표가** 대상 단위의 「줄 보기」에서
강조 범위가 됩니다 — 둘을 따로 받지 않으므로 어긋날 수가 없습니다.
...
```jsonc
{
  "meta": {
    ...
    "slug":        "YOEDSMOV",           // 필수. 작업 폴더 이름과 내보내는 파일 이름에 쓰입니다
  ...
  "findings": [                         // 필수(빈 배열 허용)
    {
      ...
      "line":        138,               // 필수. 원문 기준 줄 번호. 카드 머리에 실리는 대표 줄
      "currentLines": [[138, 148]],     // 필수. 이 제안이 **닿는 구간**. [시작, 끝] 쌍의 배열
      //   떨어져 있으면 여러 개 적습니다 — [[104, 108], [111, 111]]
      //   스크립트가 이 좌표로 원문을 잘라 「지금」 칸을 그리고,
      //   **같은 좌표를 「줄 보기」의 강조 범위로도 씁니다.**
      //   코드를 여기 옮겨 적지 마세요. (`current` 문자열은 좌표가 없는
      //   옛 JSON 을 위해 남아 있을 뿐입니다 — 새로 쓰지 마세요)
      ...
      //   ``` 펜스를 쓰면 접이식 + 복사 버튼이 되고, 펜스 밖 문장은 산문으로 그려집니다.
      //   펜스 머리의 언어 뒤에 제목을 붙일 수 있습니다:
      //       ```csharp BuildSplitParams(int row)
      //   `함수 추출` · `SqlManager 이관` · `중복 통합` 은 **펜스가 필수**입니다 —
      //   그 셋은 "어디를 어떻게 떼어내나" 가 본론이라 문장만으로는 못 옮깁니다.
...
| `currentLines` 는 **좌표**다. 코드를 옮겨 적지 않는다 | 원문은 스크립트가 잘라 씁니다. 옮겨 적으면 개행이 뭉개져 한 줄로 늘어섭니다 |
| `currentLines` 를 **메서드 전체로 잡지 않는다** | 전부 강조하면 아무것도 가리키지 못합니다. 실제로 손댈 구간만 |
| `suggestion` 의 코드는 **``` 펜스 안에** 넣는다 | 펜스 밖 문장과 섞여도 각각 제 모양으로 그려집니다. 펜스 없이 섞으면 한글 설명까지 모노스페이스가 됩니다 |
...
- **왼쪽 목차가 파일 탐색기** — 폴더를 접었다 펼 수 있고, 갈림 없는 폴더는 `a/b` 로 합칩니다
- **C# · XAML · SQL 문법 하이라이트** — 외부 라이브러리 없이 내장.
  `$@"…"` 축자 문자열 안의 여러 줄 SQL 도 SQL 키워드로 물듭니다
- 검색 — 제안 내용·파일·근거·유형·비용을 한 번에.
  (유형·비용·관점 **칩 필터는 없앴습니다** — 열네 칩이 검색창 아래를 채웠는데,
  실제로 고르는 방법은 「한눈에 보기」 표를 훑는 것이었습니다)
- 카드는 **왜 바꾸나 → 지금 코드 → 제안 코드** 세 줄기.
  `근거` · `영향` · `점검` 은 그 아래 **한 자리에 접혀** 있습니다
- **「지금 코드」가 줄번호와 함께** 실립니다. 떨어진 구간은 `⋯` 로 끊어 보여 줍니다
- **「줄 보기」가 제안이 닿는 구간 전체를 칠합니다** — 대표 줄 하나가 아닙니다
- **제안 코드는 접이식 + 복사 버튼** — 펴서 그대로 가져다 붙입니다
- 제안 내용을 마크다운으로 복사·저장
