...
이 문서의 위반은 대부분 **참고** 입니다. 다음일 때만 올립니다.

| 우선순위 | 언제 |
|---|---|
| **먼저** | 이름이 실제 동작과 **반대**여서 호출부가 오해할 때. 예: `IsValidLot()` 이 유효할 때 `false` 를 반환 |
| **다음** | 규칙 2-1 위반으로 VO 와 함수의 대응이 깨져 다음 사람이 못 찾을 때, 또는 규칙 1-2 위반이 여러 곳에 퍼져 있을 때 |
| **참고** | 그 외 전부 |

**이름이 어색하다는 느낌만으로 다음 을 주지 마세요.** 규칙 번호를 댈 수 없으면 제안하지 않습니다.
...
규칙 번호가 없으면 `V-3` 으로, 영향 범위를 세지 않은 `비용: 작음` 은 `V-7` 로 걸립니다.
