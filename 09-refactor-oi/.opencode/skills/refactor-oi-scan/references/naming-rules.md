...
컨벤션 리뷰어(`refac-convention`)가 대조하는 **유일한 기준**입니다.
여기 없는 것을 규칙이라고 제안하면 검증 단계에서 `V-3` 으로 반려됩니다.

> 이 하네스는 코드를 고치지 않습니다. **"이렇게 바꾸면 좋겠다"를 적는 자리**입니다.
> `틀렸습니다` 대신 `이렇게 하면 …가 나아집니다` 로 쓰세요.
...
이 문서에서 나오는 제안은 **전부 `이름 변경`** 입니다. 고민할 것이 없습니다.

다만 **무엇이 나아지는지**는 반드시 쓰세요 — 개발자는 한눈에 보기 표에서
제목 한 줄만 보고 고칩니다. "이름이 어색하다"는 느낌만으로는 아무도 고치지
않습니다. 규칙 번호를 댈 수 없으면 제안하지 마세요.
...
이름 바꾸기의 비용은 **호출부 수**가 거의 전부입니다. 세어서 적으세요.

| 비용 | 언제 |
|---|---|
| **작음** | 한 파일 안에서 끝남. 지역변수·private 멤버가 대부분입니다 |
| **보통** | 호출부가 여러 파일에 있지만 **대상 경로 안**입니다 |
| **큼** | `public` 이라 경로 밖에서 쓸 수 있거나, **XAML 바인딩·리소스 키가 걸려 있어** 이름을 바꾸면 마크업도 같이 바꿔야 합니다 |

> 마지막 줄이 WPF 에서 특히 중요합니다. 프로퍼티 이름은 `{Binding Foo}` 로 XAML 에 문자열로
> 박혀 있어서, C# 만 바꾸면 **컴파일은 되고 화면만 빕니다.**
> `1-index.json` 의 `wpfHints` 에 `binding` 이 있으면 비용은 최소 **보통**입니다.

## 제안할 때 반드시 넣을 것

```
근거: 규칙 1-4 (bool 반환은 Is 로 시작)
영향: 호출부 3곳 (YOEDSMOV.xaml.cs:96, :142, :208)
```

규칙 번호가 없으면 `V-3` 으로, 영향 범위를 세지 않은 `비용: 작음` 은 `V-6` 로 걸립니다.
