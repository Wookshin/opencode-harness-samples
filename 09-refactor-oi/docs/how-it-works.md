...
| **왼쪽 목차 = 파일 탐색기** | 폴더 접기. 갈림 없는 폴더는 `a/b` 로 합칩니다 |
| **C# · XAML · SQL 하이라이트** | 외부 라이브러리 없이 내장 |
| **카드는 세 줄기** | 왜 바꾸나 → 지금 코드 → 제안 코드 (근거·영향·점검은 접힘) |
| **「지금 코드」가 줄번호와 함께** | `currentLines` 좌표로 원문에서 잘라 옵니다 |
| **「줄 보기」가 구간 전체를 칠함** | 같은 좌표를 씁니다 — 둘이 어긋날 수 없습니다 |
| **제안 코드 접이식 + 복사 버튼** | ``` 펜스마다 하나씩 |
| 마크다운 내보내기 | 제안 목록을 그대로 |
