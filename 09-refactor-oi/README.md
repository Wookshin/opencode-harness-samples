...
> **Phase 2 의 게이트는 눈이 아니라 스크립트가 봅니다** —
> `ws.py --suggest-status <작업폴더>`. 제안자가 **다 쓴 것**과 **쓰다 끊긴 것**은
> 둘 다 "파일이 좀 있다" 로 보여 눈으로는 안 갈립니다. 끊긴 것을 빈 결과로
> 판정해 일하던 제안자를 버리는 사고가 실제로 났던 자리라, 판정을 기계에 넘겼습니다.
> **진행이 있는 한 계속 이어 부르고**, 파일이 두 번 그대로이거나 회차 상한에
> 닿을 때만 멈춥니다.
...
| 확신 | 언제 | 제안에 이렇게 적습니다 |
|---|---|---|
| **높음** | `private`/`internal` 이고 WPF 경로 어디에도 안 걸림 | "지워도 컴파일과 동작이 그대로입니다" |
| **중간** | `public`/`protected` — **대상 경로 밖**에서 쓸 수 있음 | "저장소 전체에서 찾아본 뒤 지우세요" |
...
| V-6 | `비용: 작음` 인데 영향을 세지 않음 → **`보통` 으로 상향** |

반려된 제안은 **지우지 않고** HTML 맨 아래 접이식 절에 남습니다. 감사 흔적입니다.

### ⑤ 두 축이 "무엇을 고를지"를 돕는다

08 은 주의 등급 하나였습니다. 여기서는 **우선순위를 아예 두지 않습니다** —
순서는 개발자가 정하기 때문입니다. 대신 두 축으로 **고르기 쉽게** 만듭니다.
...
유형에는 **경중이 없습니다.** 그래서 칩 색도 강약을 두지 않고 일곱을
구분만 합니다 — `먼저`(빨강) → `참고`(회색) 같은 스케일이 없습니다.

### ⑥ 순서를 정해 주지 않는다

**이 리포트는 개발자가 골라 쓰는 자료입니다.** 팀이 모여 순서를 정하는
자료가 아닙니다. 그래서 「손대는 순서」·「묶어서 하면 좋은 것」·
「지금은 두는 게 낫습니다」를 **전부 뺐습니다** — 하네스가 짠 계획은
어차피 쓰이지 않습니다.
...
| | |
|---|---|
| **한 줄 진단** | 무엇이 지금 가장 비싼가. 3~5줄. 오케스트레이터가 직접 씁니다 |
| **한눈에 보기 표** | 제안 전체를 한 줄씩 — ID · 우선순위 · 비용 · 단위 · 무엇이 바뀌나 |

### ⑦ 한눈에 보기 표는 authoring 하지 않는다

표는 `findings` 에서 **스크립트가 만듭니다.**

```
findings[]  →  표 (ID · 우선순위 · 비용 · 단위 · title)
```

그래서 세 가지가 공짜로 따라옵니다.

- **본문과 어긋날 수 없습니다** — 같은 데이터를 두 번 그리는 것뿐입니다
- **필터를 걸면 표도 같이 줄어듭니다** — `이름 변경` 을 누르면 그 유형만 남습니다
- **행을 누르면 그 제안 카드로 내려갑니다**

대신 `findings[].title` 이 그대로 「무엇이 바뀌나」 칸이 됩니다.
**제목 한 줄만 보고 고칠지 말지가 갈립니다** — `suggest-format.md` 5절이
그만큼 중요해졌습니다.
...
DPI mapper 저장소는 **전사 공용**입니다. dao 폴더 하나에 XML 이 수백 개 있고,
그중 한 화면이 부르는 것은 보통 서너 개입니다.

그래서 `collect.py` 는 **코드를 먼저 읽습니다.**
...
하나에 SQL 이 수천 개인 것이 보통이라, 통째로 옮기면 작업 폴더가 부풀고
건너뛰면 본문을 아무도 못 읽습니다. `<select id="…">` 의 id 로 골라 남기고
`<include refid>` 로 끌어 쓰는 `<sql>` 조각도 따라갑니다 — 1.2MB 가 1KB 가 됩니다.
...
**판정은 화이트리스트로 합니다.** `DPICALL`·`SQLEXEC` 만 SQL 을 가진
...
`sql.unusedIds` 의 뜻도 그만큼 좁아집니다 — "저장소 전체에서 아무도 안 쓴다"가 아니라
**"이 코드가 쓰는 SQL 과 같은 파일에 있는데, 이 코드는 안 쓴다"** 입니다.
참조 문서와 프롬프트가 그 선을 넘지 않도록 못 박아 두었습니다.
...
| 2 | **한 줄 진단** + **한눈에 보기 표** | 무엇이 비싼가 · 제안 전체를 한 줄씩 |
...
| 기능 | 쓰임 |
|---|---|
| **한눈에 보기 표** | 제안 전체를 한 줄씩. `findings` 에서 자동 생성 — 행을 누르면 그 카드로 |
| **왼쪽 목차 = 파일 탐색기** | 폴더를 접었다 펼 수 있습니다. 갈림 없는 폴더는 `a/b` 로 합칩니다 |
| **C# · XAML · SQL 문법 하이라이트** | 외부 라이브러리 없이 내장 |
| 검색 | 제안 내용·파일·근거·유형·비용을 한 번에 (칩 필터는 없앴습니다) |
| **카드는 세 줄기** | 왜 바꾸나 → 지금 코드 → 제안 코드. 근거·영향·점검은 아래에 접혀 있습니다 |
| **「지금 코드」가 줄번호와 함께** | `currentLines` 좌표로 원문에서 잘라 옵니다. 떨어진 구간은 `⋯` 로 끊어서 |
| **「줄 보기」가 구간 전체를 칠함** | 같은 좌표를 씁니다 — 대표 줄 하나가 아닙니다 |
| **제안 코드 접이식 + 복사 버튼** | `함수 추출` · `SqlManager 이관` 은 붙여넣을 수 있는 코드까지 옵니다 |
| **⚙ 설정 메뉴** | 마크다운 복사 · 코드 펼치기 · 테마 · 인쇄 |
...
- **mapper 폴더를 크게 만들어 보세요.** 노이즈 XML 을 300개 넣고 돌리면
  `mapper N개 (선별)` 이 1 로 나옵니다. `--all-mappers` 를 주면 301 이 됩니다.
...
│   │   ├── refac-verifier.md            Phase 3 · 제안 검증 V-1~V-6 (고가)
...
│       │   ├── suggest-format.md        3인 공통 출력 형식 · 개선 유형·비용 기준
