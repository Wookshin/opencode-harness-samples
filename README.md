...
## 단계 사이는 파일로 잇습니다 (조합 샘플 05·06·07)

에이전트끼리는 직접 대화할 수 없습니다. 각자 별도 세션에서 일하기 때문입니다.
그래서 조합 샘플들은 **`_workspace/` 폴더에 파일을 남기고, 다음 단계가 그걸 읽습니다.**
...
```bash
rm -f 0*/[_]workspace/*.md 0*/_workspace/*.js
git checkout -- .   # 샘플 소스를 원복
```
...
조합 샘플은 각각 `/plan-only`, `/repro-only`, `/round-one` 처럼 **첫 Phase 만 돌리는 커맨드**를 갖고 있습니다.
