...
- 관점 3종 · 심각도 3종 토글, 전체 검색 — 체크리스트와 본문에 **동시에** 걸립니다
- 지적마다 **[합의] [보류] [반려]** + 메모 → 브라우저에 저장 (`localStorage`)
- "결정 12 / 27" 진행 표시, **미결정만** 필터
...
```
흐름     0 폴더 확정 → 1 수집 → 2 리뷰 3인 동시 → 3 검증 → 4 리포트
잇는 법  _workspace/pr-<번호>/ 안의 파일. 프롬프트에는 경로만
게이트   스코프 없이 리뷰 X · 검증 없이 리포트 X · HTML 없이 완료 X
