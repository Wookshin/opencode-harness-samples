...
| **확인사항의 근거가 검증되지 않는다** | 안 바뀐 코드를 신규로 오인하거나, 인용한 코드가 실제와 다릅니다 | **검증관**이 확인사항을 원문과 대조해 반려합니다 (생성-검증) |
...
### Phase 3 — 확인사항 검증 (생성-검증)

`review-verifier` 가 확인사항을 **하나씩** 원문·패치·규칙 문서와 대조합니다.

| 검사 | 통과 못 하면 |
|---|---|
| **V-1** 인용한 라인이 변경단위 **안**에 있는가 | `REJECTED` — 안 바뀐 기존 코드 확인사항 |
| **V-2** 인용 코드가 `src/after/` 원문과 **글자 그대로** 같은가 | `REJECTED` — 존재하지 않는 코드 |
| **V-3** 근거로 든 규칙 번호가 참조 문서에 **실재**하는가 | `REJECTED` — 지어낸 규칙 |
| **V-4** 꼭 확인 인데 "그래서 무슨 일이 생기는지"가 있는가 | **확인 권장 로 강등** (반려 아님) |

여기에 중복 확인사항과 관점 침범도 걸러냅니다.
한 리뷰어의 반려율이 **1/3 을 넘으면** 그 리뷰어 **세션으로 되돌려** 다시 쓰게 합니다
(새 리뷰어를 부르지 않습니다 — 그 사람은 자기가 무엇을 봤는지 기억합니다). **최대 2회.**

**반려된 확인사항은 지우지 않습니다.** HTML 맨 아래 접이식 절에 사유와 함께 남습니다.
팀원이 "왜 이건 안 잡혔지"를 확인할 수 있어야 하기 때문입니다.

> 이 단계가 있는 이유는 하나입니다. **틀린 확인사항 한 건이 회의 30분을 잡아먹기 때문**입니다.
...
그래서 Phase 1 에서 좌표(`L1`, `L2` …)를 먼저 확정하고, **셋 다 그 ID 로만** 확인사항합니다.
...
그래서 `findings.json` 에는 확인사항 내용과 **좌표만** 담고, 실제 코드와 diff 색칠은
스크립트가 원문에서 잘라 계산합니다.
...
### ⑥ 에이전트가 하네스 파일을 찾지 않는다

`.opencode/` 는 **숨김 폴더**입니다. 그리고 검색 도구 대부분은 숨김 경로를 건너뜁니다.

| 방법 | `collect.py` 를 찾는가 |
|---|---|
| ripgrep 기본값 (`grep`·`glob` 도구의 바탕) | **못 찾음** |
| Python `glob('**/collect.py')` | **못 찾음** |
| `rg --hidden` · `find` | 찾음 |

그래서 에이전트가 "경로가 맞나 확인해 보자"고 검색하는 순간 **"하네스가 설치 안 됐다"**는
결론에 도달합니다. 실제로 실 저장소에서 그렇게 멈춘 적이 있습니다.

고친 방법은 기능 추가가 아니라 **금지**입니다. 에이전트 7종 전부에 같은 블록을 넣어
경로는 고정이니 확인하지 말고 그냥 쓰라고, 없으면 실행할 때 알게 된다고 못 박았습니다.
목록이 필요한 곳은 `ws.py` 가 답하고, 실패는 `collect.py` 프리플라이트가 조치까지 말합니다.
...
└── pr-1234/
    ...
    ├── 2-review-refactor.md     리팩토링 확인사항 (R###)
    ├── 2-review-feature.md      기능 확인사항 (F###)
    ├── 2-review-sql.md          SQL 확인사항 (S###) + SQL 본문 (Q###)
    ├── 3-verify.md              확인사항별 CONFIRMED / NEEDS-INFO / REJECTED
...
| **감사 흔적** | 반려된 확인사항까지 남아 회의에서 확인됩니다 |
...
| 순서 | 무엇 |
|---|---|
| 1 | **전체 변경사항 요약** — 이 PR 이 무엇을 하는지 (`diff-scoper` 가 씁니다) |
| 2 | **종합 평가** — 어디를 같이 봐야 하는지 (`review-lead` 가 씁니다) |
| 3 | **변경 요약** — 변경단위별 "무엇이 바뀌었나" + 확인사항 건수 |
| 4 | 파일 → 변경단위 → 확인사항 카드 (코드는 기본 접힘) |

회의를 굴리는 장치들:

- 검색 — 확인사항 내용·파일·근거
- **보기 전환** — 코드 변경을 `위아래`(통합) / `좌우`(나란히) 로. 선택은 브라우저에 남습니다
- **⚙ 설정 메뉴** — 리뷰 내용 마크다운 복사 · 코드 펼치기 · 보기 · 테마 · 인쇄
- **C# · XAML · SQL 문법 하이라이트** — `$@"…"` 안의 여러 줄 SQL 도 SQL 로 물들고,
  XAML 은 여러 줄에 걸친 속성·`{Binding …}` 마크업 확장까지 나눠 봅니다
- 테마 전환 (시스템 → 라이트 → 다크), 인쇄 시 필터가 사라지고 전부 펼쳐진 상태로 출력

> 이 리포트는 합격/불합격을 판정하지 않습니다. 확인사항은 `꼭 확인`·`확인 권장`·`참고`
> 세 등급으로만 표시합니다.
...
이 샘플에는 **세 관점에 각각 걸리는 결함**과, 검증에서 반려되도록 만든 **틀린 확인사항 3건**이
...
| 증상 | 원인과 대처 |
|---|---|
| **무엇이 문제인지 모르겠음** | 먼저 `/doctor`. python·git·gh·경로·쓰기 권한을 한 번에 점검하고 조치를 알려 줍니다 |
| 에이전트가 **"collect.py 가 없다"**고 함 | **있는데 못 찾은 것입니다.** `.opencode/` 는 숨김 폴더라 `grep`·`glob` 이 건너뜁니다. `/doctor` 로 실제 존재를 확인하세요. 스크립트를 새로 만들게 두지 마세요 |
| 에이전트가 `_workspace/` 를 못 찾고 맴돎 | 비어 있거나 없는 게 **첫 실행의 정상 상태**입니다. `collect.py` 가 만듭니다 |
