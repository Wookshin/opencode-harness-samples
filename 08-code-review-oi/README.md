...
폐쇄망에서 열려야 하므로 CDN 을 못 씁니다(외부 요청 0). 그래서 C# · XAML · SQL
토크나이저를 템플릿 안에 작게 넣었습니다. diff 는 줄 단위로 그려지는데, **여러 줄에 걸친
블록 주석과 축자 문자열(`@"…"`)이 끊기지 않도록 줄 사이에 상태를 들고 갑니다.**
...
WPF 라 `.xaml.cs` 옆에는 늘 `.xaml` 이 붙어 다닙니다. 같은 이유로 마크업도 물들입니다 —
XAML 은 속성이 여러 줄에 걸치는 일이 잦아서, **태그가 열린 채 줄이 넘어가는 것**까지
상태로 들고 가야 다음 줄 첫 이름을 요소가 아니라 속성으로 봅니다.

```xml
<DataGridTextColumn Header="상태"     Width="90"      ← 요소 이름 · 속성 이름 · 값
                    Binding="{Binding LOT_STATUS,     ← 줄이 넘어가도 속성으로 이어짐
                                      Mode=OneWay}" />  ← {…} 안의 확장 이름도 따로
```

`.xaml` · `.axaml` 말고 `.xml` · `.config` · `.csproj` 도 같은 토크나이저를 씁니다.
...
| 기능 | 쓰임 |
|---|---|
| **C# · XAML · SQL 문법 하이라이트** | 외부 라이브러리 없이 내장. `$@"…"` 안의 여러 줄 SQL 도 SQL 로 물듦 |
