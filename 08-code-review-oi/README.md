...
리뷰 결과를 코드 순서대로 늘어놓으면, 팀원은 **뭘 봐야 하는지 알기 위해 전부 읽어야** 합니다.
그래서 페이지 맨 위에 두 개의 표를 둡니다.

- **리뷰 체크리스트** — 지적 전체를 심각도 순으로 한 줄씩 (`findings[].title` + `basis` + 위치)
- **변경 요약** — 변경단위별 "무엇이 바뀌었나" (`units[].summary`) + 지적 건수

코드 블록은 **기본으로 접혀** 있고, 지적 카드의 `[코드 보기]` 로 그 자리만 펼칩니다.
회의는 체크리스트를 위에서 아래로 훑으며 진행하고, 코드는 필요할 때만 엽니다.

그래서 `units[].summary` 와 `findings[].title` 을 **한 줄로 읽히게 쓰는 것**이 중요합니다.
요약이 부실하면 이 구조가 무너지고 결국 코드를 다 읽게 됩니다.
...
**요약이 먼저, 코드는 나중입니다.** 코드를 다 읽지 않고도 "뭘 봐야 하는지"가 먼저 보이도록
페이지가 이 순서로 짜여 있습니다.

| 순서 | 무엇 | 쓰임 |
|---|---|---|
| 0 | **전체 변경사항 요약 · 종합 평가** | 결론부터. "이 PR 이 뭘 하는지"와 "그래서 어떤지". 회의 첫 마디가 여기서 나옵니다 |
| 1 | **리뷰 체크리스트** | 지적 전체를 심각도 순으로 한 줄씩. BLOCKER 부터 묶여 나오고 근거·위치·결정 상태가 한 행에. **행을 누르면 상세로 이동** |
| 2 | **변경 요약** | 변경단위별 "무엇이 바뀌었나" + 지적 건수. 이 PR 이 뭘 했는지 코드 없이 파악 |
| 3 | 파일 → 변경단위 → 지적 카드 | **코드 블록은 기본으로 접혀 있습니다.** 필요할 때만 펼침 |

화면을 일부러 얇게 뒀습니다. 관점·심각도 필터와 대시보드 카드를 걷어내고 **`리뷰 범위` 한 줄**만
남겼습니다. 회의에서 실제로 쓰는 것은 체크리스트와 코드이지, 통계가 아니기 때문입니다.

원래 쓰던 스킬의 `1. 전체 변경사항 요약 / 2. 상세 코드 리뷰 / 3. 종합 평가` 구조를
그대로 옮기되, **종합 평가를 맨 위로 올렸습니다.** 회의는 결론부터 여는 게 빠릅니다.
...
| **변경 유형 범례** | `신규·변경·삭제·이름변경·이동` 다섯 가지와, 유형이 아닌 `단순` 표시를 접이식으로 설명 |
| 회의 진행 순서 칩 | 종합 평가의 `agenda` — 누르면 해당 지적으로 이동. 그 순서대로 회의를 굴립니다 |
| 변경단위별 before/after 코드 (추가=초록, 삭제=빨강) | 펼쳐서 화면에 띄워 놓고 같이 봄 |
| 지적된 라인에 앵커 마커, 카드의 **[코드 보기]** | 어디 얘기 중인지 헷갈리지 않음 |
| 파일 원문 접이식 임베드 | 저장소 없이도 앞뒤 맥락 확인 |
| 지적마다 **[합의] [보류] [반려]** + 메모 | 회의 중에 바로 기록 (`localStorage`, 키가 PR 별로 갈려 두 리포트를 같이 열어도 안 섞임) |
| "결정 12 / 27" 진행 표시 | 남은 것이 몇 건인지 |
| **보기 전환** (`위아래` ↔ `좌우`) | 통합 diff 로 흐름을 보다가, 좌우로 놓고 변경 전후를 나란히 비교 |
| 검색 | 지적 내용·파일·근거를 한 번에. 체크리스트와 본문에 동시에 걸립니다 |
| 회의 결과 마크다운 복사 / JSON 저장 | 그대로 회의록에 붙여넣기 |
| SQL 본문 절 (경로 포함) | 회의 중에 DPImgr 파일을 직접 열어봄 |
| **테마 전환** (`시스템 → 라이트 → 다크`) | 회의실 프로젝터는 밝게, 자리에서 볼 땐 어둡게. 고른 값은 브라우저에 남습니다 |
| 인쇄 (`Ctrl+P`) | 필터·버튼이 사라지고 전부 펼쳐진 상태로 출력. **화면이 다크여도 종이는 밝게** |

> 체크리스트와 변경 요약이 곧 회의 자료이므로, **`findings[].title` 과 `units[].summary` 가
> 이 리포트의 얼굴**입니다. 리뷰어와 `diff-scoper` 프롬프트가 이 둘을 한 줄로 쓰게 하는 이유입니다.
