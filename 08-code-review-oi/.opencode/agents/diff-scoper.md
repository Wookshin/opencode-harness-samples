...
| `<작업폴더>/1-scope.md` | **이 PR 이 하는 일**(리포트 맨 위에 실림) + 파일별 변경 유형·우선순위·SQL 변경 여부 |
...
EDS 반출 확정을 **단건에서 다건으로** 바꾸는 PR 입니다. 그리드에서 여러 Lot 을 선택하면
반복문으로 하나씩 확정을 겁니다.

확정 전에 Lot 상태를 확인하는 검사 함수와, 선택 행에서 Lot ID 를 모으는 함수가 새로 생겼습니다.
그 과정에서 SqlManager 쪽도 함께 바뀌었습니다 — 조회 쿼리에 라인 조건이 붙었고,
상태 조회용 DPICALL SQL ID 가 교체됐습니다.

## 눈에 띄는 변경
- Confirm() 이 단건 처리에서 선택 목록 반복 처리로 전면 재작성됨 (L5)
- DPICALL SQL ID 교체: lot.selectMcLot → lot.selectMcLotWithLine (L9)
...
### 「이 PR 이 하는 일」 은 리포트 맨 위에 실립니다

HTML 리포트를 열면 **가장 먼저 보이는 것**이 이 문단입니다. 팀원들은 코드를 펼치기 전에
이걸 읽고 "무슨 PR 인지" 파악합니다.

| | |
|---|---|
| **3~5줄**, 문단을 나눠서 | 한 덩어리로 쓰면 아무도 안 읽습니다 |
| **무엇을 하는 PR 인지**부터 | `EDS 반출 확정을 단건에서 다건으로 바꾸는 PR 입니다` |
| 그다음 **무엇이 딸려 바뀌었는지** | `그 과정에서 SqlManager 쪽도 함께 바뀌었습니다 —` |
| 좋다/나쁘다는 쓰지 않습니다 | 판단은 리뷰어와 오케스트레이터의 몫입니다 |
| `**굵게**` 로 핵심만 강조 가능 | 리포트에서 굵게 렌더됩니다 |
| **식별자는 백틱으로** | `` `SqlManager` `` · `` `CheckLot()` `` — 코드 칩으로 렌더돼 문장에서 바로 구분됩니다 |

`파일 2개가 수정되었습니다` 같은 문장은 **수치일 뿐 요약이 아닙니다.** 수치는 리포트가 따로 셉니다.

`## 눈에 띄는 변경` 은 선택입니다. 있으면 2~4개, 각 줄 끝에 변경단위 ID 를 답니다.
...
**제목만 읽고도 무엇이 바뀌었는지 알 수 있어야 합니다.** 목차·변경 요약 표·본문 헤더
세 곳에 같은 문장이 실리므로, 이 한 줄이 부실하면 팀원은 결국 코드를 다 열어 봐야 합니다.

### 세 조각을 넣으세요

`무엇을` + `어떻게 했는지` + (필요하면) `그래서 어떻게 되는지`

| 나쁨 | 좋음 | 왜 |
|---|---|---|
| `MaxRowCount 필드 추가` | 최대 행 수 필드 `` `MaxRowCount` `` 추가 — 선언만 있고 쓰는 곳 없음 | 무엇을 위한 필드인지, 상태가 어떤지 |
| `GetLotStatus 수정` | `` `GetLotStatus` `` 의 DPICALL 을 `` `selectMcLot` `` → `` `selectMcLotWithLine` `` 으로 교체 | 무엇이 무엇으로 바뀌었는지 |
| `조회 쿼리 변경` | `` `GetMatIdList` `` 조회에 상태 컬럼·라인 조건 추가, `` `lot_id` `` 를 `` `TRIM()` `` 으로 감쌈 | 어떤 변경이 몇 개인지 |
| `함수 추가` | 반출 가능 여부를 판정하는 `` `CheckLot()` `` 신규 추가 | 그 함수가 무엇을 하는지 |

`코드 일부 변경`, `로직 수정`, `리팩토링` 같은 요약은 **아무것도 알려주지 않습니다.** 다시 쓰세요.

### 규칙

| | |
|---|---|
| **식별자는 백틱으로 감쌉니다** | `` `Confirm()` `` · `` `_sql.Init()` `` · `` `LOT_STATUS` `` — 리포트에서 코드로 렌더돼 눈에 띕니다 |
| 파일명이나 위치를 반복하지 않습니다 | ~~`YOEDSMOV.xaml.cs 수정`~~ — 파일은 표의 다른 칸에 이미 있습니다 |
| 한 줄, 40~60자 | 두 줄이 필요하면 변경단위를 잘못 묶은 것입니다 |
| 좋다/나쁘다는 쓰지 않습니다 | 판단은 리뷰어의 몫입니다. 사실만 적으세요 |
