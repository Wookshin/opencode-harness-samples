...
- **리네이밍 PR 에서 호출부가 안 바뀐 것** — 함수명은 바꿨는데 VO·주석·테스트가 옛 이름.
  옛 이름으로 `grep` 해서 **남아 있는 곳을 세어** `**점검 사항**` 에 적으세요.
  그 칸은 diff 밖을 가리켜도 범위 밖으로 반려되지 않습니다
- **이름과 동작이 반대인 bool 함수** — 이건 참고 가 아니라 **꼭 확인** 입니다
...
`references/review-format.md` 의 형식 그대로 씁니다. 확인사항 ID 접두사는 **`R`** 입니다.
