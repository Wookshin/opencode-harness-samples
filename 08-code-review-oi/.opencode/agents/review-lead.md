...
이 글은 **리포트 맨 위, 판정 배지 옆에** 그대로 실립니다. 회의를 여는 첫 마디가 됩니다.
...
**이대로 병합하면 안 됩니다.** 기능·SQL 양쪽에 실행 시 바로 드러나는 결함이 있습니다.

가장 큰 문제는 CheckLot 의 판정이 이름·주석과 반대라는 것입니다(R001). 이 상태로 돌리면
반출 가능한 Lot 이 전부 건너뛰어지고 대기·보류 Lot 만 확정되므로, 기능이 정반대로 동작합니다.

구조 자체는 방향이 맞습니다. 위 세 건만 정리하면 나머지는 후속으로 미뤄도 됩니다.
...
| 절 | 필수 | 내용 |
|---|---|---|
| `## 최종 의견` | **필수** | **3~5줄, 문단을 나눠서.** 병합 가부 → 가장 큰 문제 → 전체 인상 순서 |
| `## 재확인 필요사항` | 필수(없으면 `없음`) | 회의에서 **사람이 결정해야** 하는 것. 각 줄 끝에 지적 ID |
| `## 회의 진행 순서` | 필수 | 지적 ID 를 볼 순서대로. 보통 BLOCKER 부터, 연관된 것끼리 묶어서 |
