...
permission:
  edit:
    ...
    "*_workspace/*": allow  # ↑ 와 같은 뜻. 왜 두 줄인지는 아래 참고
```

> 패턴은 **git 저장소 루트 기준 상대 경로**와 매칭되고, `*` 는 `/` 를 넘어갑니다.
> `*` 가 `/` 를 넘으므로 `pr-1234/src/after/…` 같은 **하위 폴더까지 한 줄로 함께 열립니다.**

**왜 같은 뜻을 두 줄 쓰나.** `_workspace/` 가 어디에 놓이느냐가 저장소마다 다릅니다.

| 어디서 | 루트 기준 경로 | 맞는 패턴 |
|---|---|---|
| 이 샘플 | `08-code-review-oi/_workspace/…` | `*_workspace/*` |
| `.opencode/` 를 복사해 간 실무 저장소 | `_workspace/…` | `_workspace/*` |

복사본에서는 선행 `*` 가 **빈 문자열을 받아야** 하는데, 글롭 구현에 따라 안 될 수 있습니다.
그러면 에이전트가 자기 보고서조차 못 쓰면서 이유는 안 나옵니다.
한 줄 더 적어서 그 경우를 아예 없앴습니다.
